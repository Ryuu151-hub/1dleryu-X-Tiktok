 ...
 * POST /api/auth/login
 * Body: { apiKey: "sk_..." }
 * Verifies key with Zernio GET /v1/auth/verify
 * Sets httpOnly session cookie (does not store the key in the cookie).
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Zernio returns { valid, userId, authType, scope? }
    if (json.valid === false) {
    ...
    setSessionCookie(res, {
      ...
      // key fingerprint only (not the key) for display
